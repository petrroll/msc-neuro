...
def runner(exp_folder, exp, run, reg_l):
    run_1(exp_folder, exp, run, reg_l)

#
# based on bs4: fully connected layers + regularization
# - one fully connected layer + d2x (2d laplacian), essentially SGA version of STA
def run_1(exp_folder, exp, run, reg_l):
    name = f'baseline4_1fully_reg_LastD2X{reg_l}_5000'
    exp = f"{exp}x{run}"

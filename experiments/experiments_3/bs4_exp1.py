...
def runner(exp_folder, exp, run, hidden, reg_h, reg_l):
    run_1(exp_folder, exp, run, hidden, reg_h, reg_l)

#
# based on bs4: fully connected layers + regularization
# - two fully connected layers + hidden: d2x (2d laplacian), last: l2 (doesn't have spatial interpretation anymore)
def run_1(exp_folder, exp, run, hidden, reg_h, reg_l):
    name = f'baseline4_2fully_xN{hidden}_reg_Hdx{reg_h}xL2{reg_l}_5000'
    exp = f"{exp}x{run}"
    ...
    def get_hsm_params_custom(input, output, i):
        ...
        hsm_params = NDNutils.ffnetwork_params(
            ...
            layer_sizes=[int(hidden*output_shape), output_shape], # paper: 9, 0.2*output_shape
            ei_layers=[None, None],
            normalization=[0, 0], 

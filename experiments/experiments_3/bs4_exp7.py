...
def runner(exp_folder, exp, run, cd2x, max_filt, l1, c_filters):
    run_1(exp_folder, exp, run, cd2x, max_filt, l1, c_filters)

#
# fixed version of bs6: based on what/where paper (6942 Neural system identification for large populations separating “what” and “where”
# - fixed regularization on conv layers (norm2_filt is not defined for non-separable), used max_filt with similar-ish idea instead
def run_1(exp_folder, exp, run, cd2x, max_filt, l1, c_filters):
    name = f'baseline4_whatwhere_Cd2x{cd2x}xmax_filt{max_filt}_filtxl1{l1}_convsize{c_filters}_x5000'
    exp = f"{exp}x{run}"

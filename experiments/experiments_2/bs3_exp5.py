...
def runner(exp_folder, exp, run, hidden, reg_lambda, reg_type):
    run_1(exp_folder, exp, run, hidden, reg_lambda, reg_type)

#
# based on bl3:
# - different normalizations with different sizes
# - ?opens larger hidden layer?
def run_1(exp_folder, exp, run, hidden, reg_lambda, reg_type):

    name = f'baseline3_{reg_type}{reg_lambda}xN{hidden}x5000'
    exp = f"{exp}x{run}"
    ...
    def get_hsm_params_custom(input, output, i):
        ...
        hsm_params = NDNutils.ffnetwork_params(
            ...
            layer_sizes=[9, int(hidden*output_shape), output_shape], # paper: 9, 0.2*output_shape

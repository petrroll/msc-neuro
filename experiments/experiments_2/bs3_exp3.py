...
def runner(exp_folder, exp, run, hidden, dog_layers):
    run_1(exp_folder, exp, run, hidden, dog_layers)

#
# based on bl3:
# - different sizes for hidden layer & number of DoG filters
# - Note: Try again with regularization?
def run_1(exp_folder, exp, run, hidden, dog_layers):

    name = f'baseline3_DoG{dog_layers}xN{hidden}x5000'
    exp = f"{exp}x{run}"

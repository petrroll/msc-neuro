...
def runner(exp_folder, exp, run, hidden, dropout_h):
    run_1(exp_folder, exp, run, hidden, dropout_h)

#
# based on bl3:
# - try dropout normalization -> only on hidden layer
# - ?opens larger hidden layer?
def run_1(exp_folder, exp, run, hidden, dropout_h):

    name = f'baseline3_drop{dropout_h}xN{hidden}x5000'
    exp = f"{exp}x{run}"

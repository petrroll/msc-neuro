...
def runner(exp_folder, exp, run, c_size, c_filters, hidden_t, hidden_s, hidden_lt):
    run_1(exp_folder, exp, run, c_size, c_filters, hidden_t, hidden_s, hidden_lt)

#
# based on bs4: exp9 without non-linearity on first layer
def run_1(exp_folder, exp, run, c_size, c_filters, hidden_t, hidden_s, hidden_lt):
    name = f'baseline4_CDoG{c_filters}s{c_size}_{hidden_t}{hidden_s}_H{hidden_lt}_x5000'
    exp = f"{exp}x{run}"

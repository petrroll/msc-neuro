...
def runner(exp_folder, exp, region):
    ...
    hidden_s = 0.1
    hidden_lt = "normal"
    run_1(exp_folder, exp, c_size, c_filters, hidden_t, hidden_s, hidden_lt, region)

#
# based on bs4: exp9 without non-linearity on first layer
def run_1(exp_folder, exp, c_size, c_filters, hidden_t, hidden_s, hidden_lt, region):
    name = f'multieval_exp10_{region}'
    exp = f"{exp}"

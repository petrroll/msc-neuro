...
def runner(exp_folder, exp, run, hidden, dog_layers, regions, l2):
    run_1(exp_folder, exp, run, hidden, dog_layers, regions, l2)

#
# based on bl3:
# - multiple regions at the same time
# - different levels of l2 regularization
def run_1(exp_folder, exp, run, hidden, dog_layers, regions, l2):

    name = f'baseline3_{str(regions).replace(" ","")}_DoG{dog_layers}xN{hidden}_l2{l2}x5000'
    exp = f"{exp}x{run}"

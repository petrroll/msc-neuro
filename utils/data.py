...
def load_data(type, region):
    ...
    return (
        (np.squeeze(np.load(f'./Data/region{region}/{type}_inputs.npy'))*1_000_000).astype(np.float64), # data is in float64 format: 0.0-0.000255
...
def load_data_multiple(indexes, dta_type):
    ...
    for i in indexes:
        ...
        input_tr_processed = (input_tr)     # optionally add processing
